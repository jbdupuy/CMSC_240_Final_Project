...
int main(int argc, char* argv[])
{
    ...
    rng.seed(initialSeed); // sets seed - call rand_double(rng) every time you want to get a new random number.
...
void loadVehicles(vector<VehicleType> newVehicles, vector<VehicleBase*> &v, Direction d)
{
    int dirInt = static_cast<underlying_type<Direction>::type>(d); // looks at the direction to determine which index in...
    // genAmts[] and newVehicles[], as each space in those corresponds to a direction 
    if(v[0] == nullptr) // if there is space to load in something; if there is not, just ignore and try again next tick
    {
        if(genAmts[dirInt] != 0) // genAmts[] stores how much of a vehicle for a given direction is left to be generated
        {
            v[0] = v[1];
            genAmts[dirInt]--;
        }
        else if(newVehicles[dirInt] == VehicleType::car) // if generate() has determined that the type should be car
        {
            ...
            genAmts[dirInt] = 1; // how many sections are left to be generated in the generated vehicle
        }
